...
lbl_file_path_sh = r'D:\OneDrive - VUMC\Research\Ongoing Research Projects\My AI projects\AI 3D otitis media\Label sheet 20220109 Task 1 Shanghai final.xlsx'
lbl_file_path_wh = r'D:\OneDrive - VUMC\Research\Ongoing Research Projects\My AI projects\AI 3D otitis media\Task 1 lbl wh 022222.xlsx'
...
# a = os.listdir(pt_folder_sh)
# for fn in os.listdir(pt_folder_sh):
#   if len(fn.split('-')[1]) == 6:
#       pt_id, _, scan_date = fn.rpartition('-')
#       os.rename(os.path.join(pt_folder_sh, fn),
#                 os.path.join(pt_folder_sh, pt_id + '-20' + scan_date))
...
# abnormal_labels = np.array([1 for _ in range(len(rotate_imgs))])
# normal_labels = np.array([0 for _ in range(len(rotate_imgs))])
...
def train_val_test_split(df_lbl, fold = 1, val_ratio = 0.15):
    ...
    # problem_dict = {}   
    df_train = df_lbl.loc[df_lbl['Test fold'] != fold]
    df_test = df_lbl.loc[df_lbl['Test fold'] == fold]
    ...
    lbl_train = df_train['Label'].to_numpy()
    lbl_test = df_test['Label'].to_numpy() 
    ...
    # number_of_cases = list(range(img_array.shape[0]))
    # test_indices = random.sample(number_of_cases, int(img_array.shape[0]*test_ratio))
    # val_indices = random.sample(list(set(number_of_cases) - set(test_indices)), int(img_array.shape[0]*val_ratio))  
    # train_indices = list(set(number_of_cases) - set(test_indices) - set(val_indices))
...
#     rotate_img_list.append(scipy_rotate(x_train[i]))

# rotate_imgs = np.stack(rotate_img_list, axis = 0)
# plot_slices(rotate_imgs, case_num = 4)
...
def get_roc_curve(y_score, y_true):
    ...
    plt.title('Three-D AI model on middle ear and temporal bone diseases')
    plt.legend(loc="best")
...
def get_cmx(y_pred_lbl, y_true):
    ...
    ax2.annotate('Accuracy = {:.1%}\nSensitivity = {:.1%}\nSpecificity = {:.1%}\nPrecision = {:.1%}'.format(accuracy, tpr, tnr, ppv), 
...
''' read from here'''
df_lbl_sh = pd.read_excel(r'D:\OneDrive - VUMC\Research\Ongoing Research Projects\My AI projects\AI 3D otitis media\Label sheet 20220131 Task 1 Shanghai CV.xlsx', 
                             header = 0)

df_lbl_sh = df_lbl_sh.loc[df_lbl_sh['New label'] != 'remove'].reset_index(drop = True)

df_lbl_sh['New label'] = df_lbl_sh['New label'].astype(int)
...
# Define callbacks.
chckpnt_path = r"C:\Users\liy45\Desktop\3d_image_classification_task1_fold{}.h5".format(fold_num)
...
df_result.to_excel(r'C:\Users\liy45\Desktop\Fold {}\Result.xlsx'.format(fold_num), index=False)
...
df_lbl_wh = df_lbl_wh.loc[df_lbl_wh['Task 1 label after review'] != 'Remove'].reset_index(drop = True)

def gen_new_lbl_task1_wh(row):
    if row['Task 1 label after review'] == 'Check':
        return row['Review YL']
    else:
        return row['Task 1 label after review']

df_lbl_wh['Labels 220222'] =  df_lbl_wh.apply(gen_new_lbl_task1_wh, axis = 1)

# df_lbl_wh['New labels'] = df_lbl_wh['New labels'].astype(int)
# df_lbl_wh['New labels 0221'] = df_lbl_wh['New labels 0221'].astype(int)
...
''' labels'''
lbl_array_wh = np.array(df_lbl_wh['Labels 220222'])
...
fold_num = 5
chckpnt_path = r"D:\OneDrive - VUMC\Research\Ongoing Research Projects\My AI projects\AI 3D otitis media\Task 1 updated 2-6-22\Fold {}\3d_image_classification_task1_fold{}.h5".format(fold_num, fold_num)
model.load_weights(chckpnt_path)



optimal_threshold = 0.5663983

# plot_slices(images_np_wh, case_num = 4)
prediction_wh = model.predict(images_np_wh, batch_size = 10)
get_roc_curve(prediction_wh, y_test_wh) #0.68175375
...
df_result_wh.to_excel(r'C:\Users\liy45\Desktop\Result_wh_fold{}.xlsx'.format(fold_num), index=False)

plot_test_slices(images_np_wh, df_result_wh, case_selection = 203)
